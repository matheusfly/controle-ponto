import { Teacher, Student, ScheduleEntry, WorkLog, PriorityList, ShiftRoster, Announcement, TeacherType, ClassType, CapacityProfile, ForecastedWorkload, ResourceConstraint, ScheduleProposal } from '../types.ts';
...
// --- PlanMode Data ---
let capacityProfiles: CapacityProfile[] = [
  {
    id: 'cp1',
    name: 'Perfil Padrão 2025',
    teacherId: 't1', // Vitor
    availability: [
      { day: 0, isAvailable: false, startTime: '09:00', endTime: '17:00' },
      { day: 1, isAvailable: true, startTime: '06:00', endTime: '21:00' },
      { day: 2, isAvailable: true, startTime: '06:00', endTime: '21:00' },
      { day: 3, isAvailable: true, startTime: '06:00', endTime: '21:00' },
      { day: 4, isAvailable: true, startTime: '06:00', endTime: '21:00' },
      { day: 5, isAvailable: true, startTime: '06:00', endTime: '21:00' },
...
let forecastedWorkloads: ForecastedWorkload[] = [];
...
export const api = {
  ...
  async fetchInitialData() {
    ...
    return {
      ...
      forecastedWorkloads: [...forecastedWorkloads],
  ...
  // --- PlanMode API Functions ---
  async addCapacityProfile(profile: CapacityProfile) {
    await delay(100);
    capacityProfiles.push(profile);
    return profile;
  },

  async updateCapacityProfile(profileId: string, updates: Partial<CapacityProfile>) {
    await delay(100);
    let updatedProfile: CapacityProfile | null = null;
    ...
    if (!updatedProfile) throw new Error("Capacity Profile not found");
    return updatedProfile;
  },

  async createProposal(proposalData: Omit<ScheduleProposal, 'id'>) {
    await delay(100);
    const newProposal: ScheduleProposal = {
        ...proposalData,
        id: `prop-${Date.now()}`,
    };
    scheduleProposals.push(newProposal);
    return newProposal;
  },

  async updateProposal(proposalId: string, updates: Partial<ScheduleProposal>) {
    await delay(100);
    let updatedProposal: ScheduleProposal | null = null;
    scheduleProposals = scheduleProposals.map(p => {
        if (p.id === proposalId) {
